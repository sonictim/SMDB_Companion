...
impl Codec for FlacCodec {
    ...
    fn encode(&self, buffer: &AudioBuffer) -> Result<Vec<u8>> {
        ...
        // Process and write audio frames
        // First, apply mid/side processing if stereo
        let mut audio_data = buffer.data.clone();
        if buffer.channels == 2 {
            encode_mid_side(&mut audio_data);
        }

        // Now encode and write each frame
        for frame in audio_data.iter() {
            let encoded_frame =
    ...
    while let Ok(block_header) = cursor.read_u8() {
        ...
        match block_type {
            STREAMINFO_BLOCK_TYPE => {
                ...
                channels = (((cursor.read_u8()? & 0xF) >> 1) + 1) as u16;
                bits_per_sample = (((cursor.read_u8()? & 0x1F) >> 1) + 1) as u16;
...
fn decode_flac_frame_simd(
    ...
    for ch in 0..channels as usize {
        for i in (0..block_size).step_by(8) {
            let remaining = std::cmp::min(8, block_size - i);
            if remaining < 8 {
                // Handle remaining samples without SIMD
                for j in 0..remaining {
                    ...
                    samples[ch][i + j] = sample;
                }
            } else {
                // Use SIMD for full chunks
                match bits_per_sample {
                    16 => {
                        ...
                        // Convert i16 values to i32, then to f32
                        let mut i32_values = [0i32; 8];
                        for j in 0..8 {
                            i32_values[j] = values[j] as i32;
                        }

                        let simd_i32 = i32x8::from(i32_values);
                        let simd_f32 = f32x8::from(simd_i32.to_array().map(|v| v as f32));
                        // Calculate result by dividing each value by I16_DIVISOR
                        let result = f32x8::from(simd_f32.to_array().map(|v| v / I16_DIVISOR));

                        let result_array = result.to_array();
                        samples[ch][i..i + 8].copy_from_slice(&result_array);
                    }
                    24 => {
                        ...
                        let simd_i32 = i32x8::from(values);
                        let simd_f32 = f32x8::from(simd_i32.to_array().map(|v| v as f32));
                        let result = f32x8::from(simd_f32.to_array().map(|v| v / I24_DIVISOR));
...
fn validate_crc(frame_data: &[u8]) -> Result<()> {
    if frame_data.len() < 2 {
        return Err(anyhow!("Frame data too short for CRC validation"));
    }

    // Read expected CRC from the last 2 bytes of the frame
    let expected_crc = u16::from_le_bytes([
        frame_data[frame_data.len() - 2],
        frame_data[frame_data.len() - 1],
    ]);

    // Calculate CRC for all bytes except the last 2
    let calculated_crc =
...
//         let codec = FlacCodec;
//         let encoded = codec.encode(&buffer).unwrap();
