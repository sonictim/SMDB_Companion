    ...
    function filterItems(items: FileRecord[], filter: string): FileRecord[] {
        switch (filter) {
            ...
            case "Relevant":
                return items.filter(
                    (item) =>
                        !item.algorithm.includes("Keep") ||
                        item.algorithm.length > 1,
                );
            case "Keep":
    ...
    async function fetchData() {
        try {
            ...
            total = results.filter(
                (item) => !item.algorithm.includes("Keep"),
            ).length; // Fetch total count first
    ...
    async function removeRecords() {
        ...
        if (idsToRemove.length > 0) {
            ...
            await invoke("remove_records", {
                ...
                .then(async () => {
                    console.log(
                        "Successfully removed records with IDs:",
                        idsToRemove,
                    );
...
<div class="block">
        ...
        <div style="margin-left: auto; display: flex; gap: 20px;">
            ...
            {:else}
                <button class="cta-button cancel" on:click={replaceMetadata}>
                    ...
                    <span
                        >Replace '{metadata.find}' with '{metadata?.replace ||
                            ""}'</span
                    >
                </button>
            {/if}
    ...
    <div class="bar" style="margin-bottom: 16px; margin-top: 10px">
        ...
        <button
            type="button"
            class="grid item"
            on:click={toggle_enable_selections}
        >
            {#if enableSelections}
            ...
            {:else}
                ...
                >
                    {#if $metadataStore.mark_dirty}
                        <CheckSquare
                            size={20}
                            class="checkbox checked {metadata.column ==
                                'FilePath' ||
