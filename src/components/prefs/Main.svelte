<script lang="ts">
  ...
  import { preferencesStore, resetPreferences } from "../../stores/preferences";
  ...
  let options = [
    ...
    {
      id: "basic",
      name: "Remove Records From: Original Database/New Database",
      enabled: true,
    },
    {
      id: "filename",
      name: "Duplicate Files: Keep/Trash/Delete ",
      enabled: false,
    },
...
<div class="block">
  ...
  <div class="block inner">
    <div class="grid">
      ...
      <span
        >New Database Tag: <input class="input-field" placeholder="-thinned" />
      </span>
  ...
  <div class="bar" style="margin-top: 20px;">
    ...
    />
    <button class="cta-button small"> Load: </button>
    <select class="select-field" style=" margin-right: 10px;">
      {#each preset as p}
        <option value={p}>{p}</option>
      {/each}
    </select>
    <button class="cta-button small cancel"> Delete </button>
  </div>
</div>
